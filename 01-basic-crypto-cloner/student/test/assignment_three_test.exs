defmodule AssignmentTest do
  ...
  test "CoindataCoordinator returns list of currency pairs" do
    ...
    procs = CoindataCoordinator.retrieve_coin_processes()
    assert Enum.all?(procs, &is_tuple/1)
    assert Enum.all?(procs, fn {bin, pid} -> is_binary(bin) and is_pid(pid) end)
    assert length(procs) > 80
  end

  test "CoindataCoordinator its output is good and unique" do
    list = CoindataCoordinator.retrieve_coin_processes()
    assert all_processes_in_processmanager_good?(list)
  end

  test "CoindataCoordinator processes gets restarted. CoindataCoordinator has no invalid state." do
    ...
    list |> Enum.random() |> elem(1) |> Process.exit(:kill)
    l4 = length(list)
    Process.whereis(CoindataCoordinator) |> Process.exit(:kill)
    :timer.sleep(100)
    list = CoindataCoordinator.retrieve_coin_processes()
    l5 = length(list)
